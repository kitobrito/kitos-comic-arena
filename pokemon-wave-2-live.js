(function (root, factory) {
    ...
    const character = (id, name, role, folder, face, description, skills, startStatuses = []) => ({
        ...
        roleCategory: role.toLowerCase(), facePicture: img(folder, face), characterdeescription: description,
        description, descriptionHtml: description, skills, startStatuses,
    });
    const evolved = (base, next) => ({ ...base, evolvesTo: next });

    return [
        character('clefairy', 'Clefairy', 'Support/Specialist', 'clefairy', 'fp.webp',
        ...
        character('jigglypuff','Jigglypuff','Controller/Specialist','jigglypuff','fp.webp',
            'A countdown controller that accelerates Perish Song and evolves into Wigglytuff after completing its execution.', [
            evolved(
                skill('jigglypuff-perish-song','Perish Song','jigglypuff','parishsong.webp','Marks one enemy for 5 turns. When it expires, they are instantly defeated. Ends if Jigglypuff dies.', ['Random','Random','Random'],0,'single-enemy',['Mental','Ranged','Instant'],[status('jigglypuff_perish_song',5,'target',{harmful:true,visible:true,unremovable:true,endIfSourceDies:true,instantKillOnExpire:true,uniqueEnemyMarkFromSource:true})]),
                skill('wigglytuff-perish-song','Wigglytuff Perish Song','jigglypuff','wigglytuffparishsong.webp','Marks one enemy for 4 turns. When it expires, they are instantly defeated. Ends if Wigglytuff dies.', ['Random','Random','Random'],0,'single-enemy',['Mental','Ranged','Instant'],[status('jigglypuff_perish_song',4,'target',{harmful:true,visible:true,unremovable:true,endIfSourceDies:true,instantKillOnExpire:true,uniqueEnemyMarkFromSource:true})])
            ),
            evolved(
                skill('jigglypuff-sing','Sing','jigglypuff','sing.webp','Stuns all enemy harmful skills for 1 turn and advances Perish Song once.', ['Ninjutsu','Random'],3,'all-enemy',['Mental','Ranged','Instant'],[status('jigglypuff_sing',1,'all-enemy',{cannotUseHarmfulSkills:true,harmful:true}),{type:'accelerate_perish',scope:'all-enemy'}]),
                skill('wigglytuff-sing','Wigglytuff Sing','jigglypuff','wigglytuffsing.webp','Stuns all enemy harmful skills for 2 turns and advances Perish Song once.', ['Ninjutsu','Ninjutsu','Random'],3,'all-enemy',['Mental','Ranged','Instant'],[status('jigglypuff_sing',2,'all-enemy',{cannotUseHarmfulSkills:true,harmful:true}),{type:'accelerate_perish',scope:'all-enemy'}])
            ),
            evolved(
                skill('jigglypuff-wish','Wish','jigglypuff','wish.webp','Next turn, Jigglypuff or one ally heals 25 HP. Before it heals, a Perish Song target using a harmful skill on them advances its countdown once.', ['Random'],2,'self-or-single-ally',['Mental','Instant','Invisible'],[status('jigglypuff_wish',1,'target',{turnStartHeal:25,onOwnerTargetedBySkillTrigger:true,onOwnerTargetedByEnemyOnly:true,onOwnerTargetedByRequireNewSkill:true,wishAdvancePerishOnHarmful:true})]),
                skill('wigglytuff-wish','Wigglytuff Wish','jigglypuff','wigglytuffwish.webp','Next turn, Wigglytuff\'s whole team heals 25 HP. Before it heals, a Perish Song target using a harmful skill on them advances its countdown once.', ['Random','Random'],2,'all-allies',['Mental','Instant','Invisible'],[status('jigglypuff_wish',1,'all-allies',{turnStartHeal:25,onOwnerTargetedBySkillTrigger:true,onOwnerTargetedByEnemyOnly:true,onOwnerTargetedByRequireNewSkill:true,wishAdvancePerishOnHarmful:true})])
            ),
            evolved(
                skill('jigglypuff-humiliate','Humiliate','jigglypuff','humilate.webp','If the target uses a new harmful skill this turn, gain 1 Random chakra and advance Perish Song once.', ['Random'],2,'single-enemy',['Physical','Melee','Instant','Invisible'],[status('jigglypuff_humiliate',1,'target',{harmful:true,triggerOnOwnerUseSkill:true,onOwnerUseSkillHarmfulOnly:true,perishAcceleration:true,gainRandomChakraForSource:true})]),
                skill('wigglytuff-humiliate','Wigglytuff Humiliate','jigglypuff','wigglytuffhumilate.webp','If the target uses any new skill this turn, gain 1 Random chakra and advance Perish Song once.', ['Random'],2,'single-enemy',['Physical','Melee','Instant','Invisible'],[status('jigglypuff_humiliate',1,'target',{harmful:true,triggerOnOwnerUseSkill:true,perishAcceleration:true,gainRandomChakraForSource:true})])
            ),
        ...
        character('beedrill','Beedrill','Assassin','beedrill','FP.png','A stacking affliction assassin that evolves into Mega Beedrill after using Envenom twice.',[
            evolved(skill('beedrill-poison-sting','Poison Sting','beedrill','poisonsting.webp','Permanently adds 5 stacking affliction damage to one enemy each turn.', ['Random'],0,'single-enemy',['Affliction','Ranged','Instant'],[status('beedrill_poison_sting',999,'target',{infiniteDuration:true,harmful:true,turnStartDamage:5,afflictionDamage:true,mergeNumericAddKeys:['turnStartDamage'],poisonStingStacks:1,stackMetadataKey:'poisonStingStacks',stackDelta:1})]), skill('mega-beedrill-poison-sting','Mega Poison Sting','beedrill','poisonsting.webp','Deals 10 affliction damage immediately, then permanently adds 5 stacking affliction damage to that enemy each turn.', ['Random'],0,'single-enemy',['Affliction','Ranged','Instant'],[damage(10,'target',{afflictionDamage:true}),status('beedrill_poison_sting',999,'target',{infiniteDuration:true,harmful:true,turnStartDamage:5,afflictionDamage:true,mergeNumericAddKeys:['turnStartDamage'],poisonStingStacks:1,stackMetadataKey:'poisonStingStacks',stackDelta:1})])),
        ...
        character('articuno','Articuno','Mage','articuno','fp.png','A legendary ice mage that chains cooldown paralysis and stuns into an escalating Sheer Cold.',[
            skill('articuno-blizzard','Blizzard','articuno','blizzard.png','Deals 15 damage to all enemies and paralyzes their cooldowns for 1 turn.', ['Genjutsu'],1,'all-enemy',['Energy','Ranged','Instant'],[damage(15,'all-enemy'),status('articuno_blizzard',1,'all-enemy',{harmful:true,paralyzeCooldowns:true})]),
            ...
            skill('articuno-sheer-cold','Sheer Cold','articuno','sheercold.png','Casts Blizzard then Ice Beam on the enemy team and permanently gains 5 damage each use.', ['Genjutsu','Ninjutsu','Random'],2,'all-enemy',['Energy','Affliction','Ranged','Instant'],[{type:'articuno_sheer_cold',scope:'all-enemy'}]),
        ...
        character('moltres','Moltres','AOE DPS','moltres','FP.png','A legendary fire attacker that builds Heat and spends it on a team-wide Overheat that weakens with every use.',[
            ...
            skill('moltres-sunny-day','Sunny Day','moltres','sunnyday.png','For 2 turns, enemies take 3 additional affliction damage. Moltres gains 1 Heat at each turn start.', ['Bloodline'],4,'all-enemy',['Energy','Ranged','Instant','Bypassing'],[status('moltres_sunny_day_enemy',2,'all-enemy',{harmful:true,additionalAfflictionDamageTaken:3}),status('moltres_sunny_day_heat',2,'self',{turnStartGainHeat:1})]),
            ...
            skill('moltres-overheat','Overheat','moltres','overheat.png','Consumes all Heat to deal 15 affliction damage per Heat to all enemies. Damage per Heat permanently falls by 5 after every use, to 0.', ['Bloodline','Bloodline','Random'],0,'all-enemy',['Affliction','Ranged','Instant'],[{type:'moltres_overheat',scope:'all-enemy'}]),
        ...
        ], [{statusId:'moltres_heat',sourceSkillId:'moltres-overheat',duration:999,metadata:{infiniteDuration:true,unremovable:true,heat:0,overheatPenalty:0}}]),
        ...
        character('mew','Mew','Support/Controller','mew','fp.png','A mythical support that builds permanent barriers and converts accumulated shields into maximum HP.',[
        ...
        character('dragonite','Dragonite','Tank/Controller','dragonite','fp.png','A durable controller that taunts enemies and refreshes Pressure whenever it uses a skill.',[
            ...
            skill('dragonite-pressure','Passive: Pressure','dragonite','passive.png','After Dragonite uses a new skill, it gains 10 unpierceable damage reduction for 2 turns. It refreshes and never stacks.',[],0,'',['Passive','Instant'],[])
